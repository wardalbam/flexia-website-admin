...
export default function ApplyPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE || 'http://localhost:3000'}/api/applications`, {
        method: 'POST',
    ...
    <div className="max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex gap-2">
          <button disabled={loading} className="bg-blue-600 text-white px-3 py-2 rounded">{loading ? 'Verzenden...' : 'Verzenden'}</button>
          <button type="button" onClick={() => router.push('/vacatures')} className="px-3 py-2 border rounded">Annuleren</button>
        </div>
      </form>
    </div>
  )
